...
$(document).ready(function () {
  ...
  if ($("#hf_base_url").length > 0 && $("#lang_code").length > 0) {
    $.getJSON(
      $("#hf_base_url").val() + "/public/frontend_js.json",
      function (data) {
  ...
  $('.products-brands-list .carousel[data-type="multi"] .item').each(
    function () {
      var next = $(this).next();
      if (!next.length) {
  ...
  if ($("#blog-cat-content-main .category-accordian").length > 0) {
    var selectIds = $(
      "#blog-cat-content-main .category-accordian .panel-collapse"
    );
    selectIds.on("show.bs.collapse hidden.bs.collapse", function () {
      $(this).prev().find("span i").toggleClass("fa-plus fa-minus");
    });
  ...
  if ($("#product-category #price_range").length > 0) {
    $("#product-category #price_range")
      ...
      .on("slideStop", function (ev) {
        ...
        $(".price-slider-option .tooltip-inner").html(
  ...
  if ($("#product-category .product-categories-accordian h2 span").length > 0) {
    $("#product-category .product-categories-accordian h2 span").click(
      function () {
        $(
          "#product-category .product-categories-accordian .category-accordian"
        ).slideToggle("slow");
        $(
          "#product-category .product-categories-accordian h2 span"
        ).toggleClass("responsive-accordian responsive-accordian-open");
      }
    );
  }
  if (
    $("#blog-cat-content-main .blog-categories-accordian h2 span").length > 0
  ) {
    $("#blog-cat-content-main .blog-categories-accordian h2 span").click(
      function () {
        $(
          "#blog-cat-content-main .blog-categories-accordian .category-accordian"
        ).slideToggle("slow");
        $(
          "#blog-cat-content-main .blog-categories-accordian h2 span"
        ).toggleClass("responsive-accordian responsive-accordian-open");
      }
    );
  }
  if ($("#product-category .price-filter h2 span").length > 0) {
    $("#product-category .price-filter h2 span").click(function () {
      $("#product-category .price-filter .price-slider-option").slideToggle(
        "slow"
      );
      $("#product-category .price-filter h2 span").toggleClass(
        "responsive-accordian responsive-accordian-open"
      );
    });
  }
  if ($("#product-category .sort-filter h2 span").length > 0) {
    $("#product-category .sort-filter h2 span").click(function () {
      $("#product-category .sort-filter .sort-filter-option").slideToggle(
        "slow"
      );
      $("#product-category .sort-filter h2 span").toggleClass(
        "responsive-accordian responsive-accordian-open"
      );
    });
  }
  if ($("#product-category .colors-filter h2 span").length > 0) {
    $("#product-category .colors-filter h2 span").click(function () {
      $("#product-category .colors-filter .colors-filter-option").slideToggle(
        "slow"
      );
      $("#product-category .colors-filter h2 span").toggleClass(
        "responsive-accordian responsive-accordian-open"
      );
    });
  }
  if ($("#product-category .size-filter h2 span").length > 0) {
    $("#product-category .size-filter h2 span").click(function () {
      $("#product-category .size-filter .size-filter-option").slideToggle(
        "slow"
      );
      $("#product-category .size-filter h2 span").toggleClass(
        "responsive-accordian responsive-accordian-open"
      );
    });
  }
  if ($("#tag_single_page_main .tags-product-list h2 span").length > 0) {
    $("#tag_single_page_main .tags-product-list h2 span").click(function () {
      $("#tag_single_page_main .tags-product-list .tag-list").slideToggle(
        "slow"
      );
      $("#tag_single_page_main .tags-product-list h2 span").toggleClass(
        "responsive-accordian responsive-accordian-open"
      );
    });
  }
  ...
  $(".blog-reviews-content .rating-select .btn").click(function () {
    if ($(this).hasClass("blog-selected")) {
      $(".blog-reviews-content .rating-select .blog-selected").removeClass(
        "blog-selected"
      );
    } else {
      $(".blog-reviews-content .rating-select .blog-selected").removeClass(
      ...
      if ($(".blog-reviews-content #selected_rating_value").length > 0) {
        $(".blog-reviews-content #selected_rating_value").val(
          $(this).data("rating_value")
        );
      }
    }
  });
  if ($("#product_single_page").length > 0) {
    ...
    if (
      hash &&
      (hash == "#product_description_bottom_tab" || hash == "#new_comment_form")
    ) {
      product_review_tab_control_with_hashtag();
    }
  }
  if ($("#subscribtion_submit").length > 0) {
    $("#subscribtion_submit").on("click", function () {
      ...
      var str = false;
      if (
        $("#subscribe_options_name").length > 0 &&
        ($("#subscribe_options_name").val().length == 0 ||
      ...
      if (
        $("#subscribe_options_email").length > 0 &&
        ($("#subscribe_options_email").val().length == 0 ||
          $("#subscribe_options_email").val() == "" ||
          !isValidEmail($("#subscribe_options_email").val()))
      ) {
      ...
      if (!str) {
        ...
        if (
          $("#subscription_type").length > 0 &&
          $("#subscription_type").val().length > 0
        ) {
  ...
  btnnext.click(function (e) {
    ...
    if (checkoutStepValidation()) {
      if (current < widget.length) {
        widget.show();
        if (
          $("#selected_user_mode").val().length > 0 &&
          $("#selected_user_mode").val() == "login_user" &&
          !is_enable_selected &&
          $("#is_user_login").val() == false
        ) {
          var get_authentication_index =
            parseInt(
          ...
          $("#selected_user_mode").val() == "login_user" &&
          !is_enable_selected &&
          $("#is_user_login").val() == true
        ) {
          var get_login_user_address_index =
            parseInt(
              $("#checkout_page .checkout-content")
                .find("#login_user_address")
                .index()
            ) - 1;
          var get_payment_index =
            parseInt(
              $("#checkout_page .checkout-content").find("#payment").index()
            ) - 1;
          ...
          var get_guest_user_address_index =
            parseInt(
              $("#checkout_page .checkout-content")
                .find("#guest_user_address")
                .index()
            ) - 1;
          var get_payment_index =
            parseInt(
  ...
  if ($("#checkout_page #user_mode .checkout-process-user-mode").length > 0) {
    $(
      '#checkout_page #user_mode .checkout-process-user-mode input[type="radio"]'
    ).on("ifClicked", function (event) {
      $("#selected_user_mode").val(this.value);
    });
  }
  if (
    $("#account_bill_phone_number").length > 0 ||
    $("#account_shipping_phone_number").length > 0 ||
    $("#account_bill_zip_or_postal_code").length > 0 ||
    $("#account_shipping_zip_or_postal_code").length > 0 ||
    $("#account_bill_fax_number").length > 0 ||
    $("#account_shipping_fax_number").length > 0
  ) {
    $(
      "#account_bill_phone_number, #account_shipping_phone_number, #account_bill_zip_or_postal_code, #account_shipping_zip_or_postal_code, #account_bill_fax_number, #account_shipping_fax_number"
    ).ForceNumericOnly();
  }
  if ($("#apply_coupon_post").length > 0) {
    $("#apply_coupon_post").on("click", function (e) {
      e.preventDefault();
      if (
        $("#apply_coupon_code").val().length == 0 &&
    ...
    $(".chk-colors-filter").on("ifChanged", function (event) {
      if (event.currentTarget.checked) {
        if (
          ...
          $("#selected_colors").val().length > 0
        ) {
          $("#selected_colors").val(
            $("#selected_colors").val() + "," + event.currentTarget.value
          );
        } else {
          $(".filter-panel .colors-filter").append(
            '<input name="selected_colors" id="selected_colors" type="hidden">'
          );
          $("#selected_colors").val(event.currentTarget.value);
        }
    ...
    $(".chk-size-filter").on("ifChanged", function (event) {
      if (event.currentTarget.checked) {
        if (
          ...
          $("#selected_sizes").val().length > 0
        ) {
          $("#selected_sizes").val(
            $("#selected_sizes").val() + "," + event.currentTarget.value
          );
        } else {
          $(".filter-panel .size-filter").append(
            '<input name="selected_sizes" id="selected_sizes" type="hidden">'
          );
          $("#selected_sizes").val(event.currentTarget.value);
        }
      } else if (!event.currentTarget.checked) {
  ...
  if ($(".vendor-special-products-menu").length > 0) {
    $("#vendor_home_content ul.vendor-special-products-menu li a").on(
      "click",
      function () {
        $(this)
          .parents(".vendor-special-products-menu")
          .find(".active")
          .removeClass("active");
  ...
  pageLoad: function () {
    ...
    if ($("#product-category").length > 0) {
      ...
      $(".sort-by-filter")
        .select2()
        .on("change", function () {
          window.location.href = commonReplaceUrlParam(
    ...
    if (
      $('#cart_page .cart-total-content input[type="radio"]').length > 0 ||
      $('#checkout_page .cart-total-content input[type="radio"]').length > 0
    ) {
    ...
    if ($(".payment-options").length > 0) {
      $('.payment-options input[type="radio"]').on(
        "ifClicked",
        function (event) {
          if (this.value === "paypal") {
            $(".place-order").text(
              frontendLocalizationString.proceed_to_payPal
            );
            $(
              "#bacsPopover, #codPopover, #stripePopover, #twocheckoutPopover"
            ).hide();
            $("#paypalPopover").show();
          } else if (this.value === "bacs") {
            $(".place-order").text(frontendLocalizationString.place_order);
            $(
              "#paypalPopover, #codPopover, #stripePopover, #twocheckoutPopover"
            ).hide();
            $("#bacsPopover").show();
          } else if (this.value === "cod") {
            $(".place-order").text(frontendLocalizationString.place_order);
            $(
              "#paypalPopover, #bacsPopover, #stripePopover, #twocheckoutPopover"
            ).hide();
            $("#codPopover").show();
          } else if (this.value === "stripe") {
            $(".place-order").text(
              frontendLocalizationString.proceed_to_stripe
            );
            $(
              "#paypalPopover, #bacsPopover, #codPopover, #twocheckoutPopover"
            ).hide();
            $("#stripePopover").show();
          } else if (this.value === "2checkout") {
            $(".place-order").text(
              frontendLocalizationString.proceed_to_2checkout
            );
            $(
              "#paypalPopover, #bacsPopover, #codPopover, #stripePopover"
            ).hide();
...
shopist_frontend.event = {
  filterProducts: function () {
    if ($("#filterProductsByName").length > 0) {
      $("#filterProductsByName").click(function () {
        shopist_frontend.ajaxCall.filterProductsByName(
          $("#hf_base_url").val() + "/ajax/filter_products"
        );
      });
    }
  },
  ...
  shipping_method_dropdown_option: function () {
    $(
      "#cart_page #shipping_method_dropdown, #checkout_page #shipping_method_dropdown"
    )
      .select2()
      .on("change", function () {
        shopist_frontend.ajaxCall.setCartTotalByShippingMethodValue(
          $(this).val()
        );
      });
  },
  shipping_method_radio_option: function () {
    $(
      '#cart_page .cart-total-content input[type="radio"], #checkout_page .cart-total-content input[type="radio"]'
    ).on("ifClicked", function (event) {
  ...
  manageRequestProducts: function () {
    if ($("#request_product_data_submit").length > 0) {
      $("#request_product_data_submit").on("click", function () {
        var errorStr = "";
        if (
          $("#request_product_name").val().length == 0 ||
          $("#request_product_name").val() == ""
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_name +
            "</p>";
        }
        if (
          $("#request_product_email").val().length == 0 ||
          $("#request_product_email").val() == ""
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_email +
            "</p>";
        }
        if (
          $("#request_product_phone_number").val().length == 0 ||
          $("#request_product_phone_number").val() == ""
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_phone_number +
            "</p>";
        }
        if (
          $("#request_product_description").val().length == 0 ||
          $("#request_product_description").val() == ""
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_desc +
            "</p>";
        }
        if (
          $("#request_product_email").val().length > 0 &&
          !isValidEmail($("#request_product_email").val())
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_valid_email +
            "</p>";
        }
        if (
          $("#request_product_phone_number").val().length > 0 &&
          !validatePhone($("#request_product_phone_number").val())
        ) {
          errorStr +=
            '<p class="error">' +
            frontendLocalizationString.enter_valid_phone_number +
            "</p>";
        }
        if (errorStr && errorStr != "") {
          $(".request-field-message").remove();
          var validationStr =
            '<div class="request-field-message">' + errorStr + "</div>";
          $(".request-content-main").before(validationStr);
          return false;
        }
        if (errorStr == "") {
          $(".request-field-message").remove();
          shopist_frontend.ajaxCall.sendProductsRequestData(
            $(this).data("id"),
            $("#request_product_name").val(),
            $("#request_product_email").val(),
            $("#request_product_phone_number").val(),
  ...
  multi_lang_process: function () {
    $(".language-list .dropdown-content a").on("click", function (e) {
      e.preventDefault();
      shopist_frontend.ajaxCall.setMultiLangAndChangeView(
        $(this).data("lang_name")
      );
    });
  },
  multi_currency_process: function () {
    $(".change-multi-currency .dropdown-content a").on("click", function (e) {
      e.preventDefault();
      shopist_frontend.ajaxCall.setMultiCurrencyAndChangeView(
        $(this).data("currency_name")
      );
    });
  },
...
shopist_frontend.ajaxCall = {
  ...
  setCartTotalByShippingMethodValue: function (val) {
    ...
    $.ajax({
      url:
        $("#hf_base_url").val() + "/ajax/cart-total-update-by-shipping-method",
      type: "POST",
      cache: false,
      datatype: "html",
      headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content") },
  ...
  sendProductsRequestData: function (
    product_id,
    product_name,
    email,
    phone_number,
    ...
    $("#request_product_modal").append(
      '<div class="shadow-layer-on-popup"></div>'
    );
    $("#loader-1").show();
    $.ajax({
      ...
      success: function (data) {
        if (data.status == "saved") {
          ...
          $(".request-field-message").remove();
          $(".request-content-main").before(
            '<div class="request-field-message"><p class="success">' +
              frontendLocalizationString.request_saved_msg +
              "</p></div>"
          );
        }
      },
  ...
  sendSubscriptionData: function (obj, name, email, type) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (
          $("#subscriptions_modal .subscribe-content")
            .parents(".modal__content")
            .find(".modal-content-msg").length > 0
        ) {
          $("#subscriptions_modal .subscribe-content")
            .parents(".modal__content")
            .find(".modal-content-msg")
            .remove();
        }
        if (
          $("#subscribe_options_email").length > 0 &&
        ...
        if (data.status == "saved") {
          $("#subscriptions_modal .subscribe-content").before(
            '<p class="modal-content-msg">' +
              frontendLocalizationString.subscribe_success_msg +
              "</p>"
          );
        } else if (data.status == "error") {
          $("#subscriptions_modal .subscribe-content").before(
            '<p class="modal-content-msg">' +
              frontendLocalizationString.subscribe_error_msg +
              "</p>"
          );
        }
      },
  ...
  applyCoupon: function (val) {
    var msgStr =
      '<div class="alert alert-danger" style="margin-left:-15px; margin-right:15px;"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">Ã—</button><div class="message-header"><i class="fa fa-exclamation-triangle"></i>&nbsp;<strong>' +
      frontendLocalizationString.error_message_text +
      '</strong></div><p class="error-msg-coupon"></p></div>';
    $(".cart-total-area-overlay").show();
    $("#loader-1-cart").show();
    $.ajax({
      ...
      success: function (data) {
        if (
          $("#cart_page, #checkout_page").find(".error-msg-coupon").length > 0
        ) {
          $("#cart_page, #checkout_page")
            .find(".error-msg-coupon")
        ...
        if (data.error == true && data.error_type == "no_coupon_data") {
          ...
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.coupon_not_exists_msg);
        } else if (
          data.error == true &&
          data.error_type == "less_from_min_amount" &&
          data.min_amount
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(
              frontendLocalizationString.coupon_min_spend_msg +
                " " +
                data.min_amount
            );
        } else if (
          data.error == true &&
          data.error_type == "exceed_from_max_amount" &&
          data.max_amount
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(
              frontendLocalizationString.coupon_max_spend_msg +
                " " +
                data.max_amount
            );
          ...
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.coupon_no_login_msg);
        } else if (
          data.error == true &&
          data.error_type == "user_role_not_match" &&
          data.role_name
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            ...
            .html(
              data.role_name +
                " " +
                frontendLocalizationString.coupon_no_login_msg
            );
        } else if (data.error == true && data.error_type == "coupon_expired") {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.coupon_expired_msg);
        } else if (
          data.error == true &&
          data.error_type == "coupon_already_apply"
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.coupon_already_sxist_label);
        } else if (
          data.success == true &&
          (data.success_type == "discount_from_product" ||
            data.success_type == "percentage_discount_from_product" ||
            data.success_type == "percentage_discount_from_product" ||
            data.success_type == "discount_from_total_cart" ||
            data.success_type == "percentage_discount_from_total_cart")
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.coupon_added_msg);
          $(
            "#cart_page .cart-grand-total, #checkout_page .cart-grand-total"
          ).before(
            '<div class="cart-coupon"><div class="label">' +
              frontendLocalizationString.coupon_label +
              '</div><div class="value"> - ' +
              data.discount_price +
              '</div><div><button class="remove-coupon btn btn-default btn-xs" type="button">' +
              frontendLocalizationString.remove_coupon_label +
              "</button></div></div>"
          );
          $(
            "#cart_page .cart-total-content .cart-grand-total .value, #checkout_page .cart-total-content .cart-grand-total .value"
          ).html(data.grand_total);
          shopist_frontend.event.remove_user_coupon();
        } else if (
          data.error == true &&
          data.error_type == "exceed_from_cart_total"
        ) {
          $("#cart_page .cart-data, #checkout_page .cart-data").prepend(msgStr);
          $("#cart_page .cart-data, #checkout_page .cart-data")
            .find(".error-msg-coupon")
            .html(frontendLocalizationString.exceed_from_cart_total_msg);
        }
        $(".cart-total-area-overlay").hide();
        $("#loader-1-cart").hide();
      },
  ...
  removeCoupon: function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.success == true) {
          if (
            $("#cart_page, #checkout_page").find(".cart-data .error-msg-coupon")
              .length > 0
          ) {
            $("#cart_page, #checkout_page")
              .find(".cart-data .error-msg-coupon")
              .parents("li")
              .remove();
          }
          $(
            "#cart_page .cart-total-content .cart-coupon, #checkout_page .cart-total-content .cart-coupon"
          ).remove();
          $(
            "#cart_page .cart-total-content .cart-grand-total .value, #checkout_page .cart-total-content .cart-grand-total .value"
          ).html(data.grand_total);
  ...
  setUserWishlistDetails: function (data) {
    $.ajax({
      ...
      success: function (data) {
        if (
          data.status == "success" &&
          data.notice_type == "user_wishlist_saved"
        ) {
          swal(
            {
              title: "",
              ...
              confirmButtonText:
                frontendLocalizationString.wishlist_items_label,
              closeOnConfirm: false,
              imageUrl:
                $("#hf_base_url").val() + "/public/images/thumbs-up.jpg",
            },
            function () {
              location.href =
                $("#hf_base_url").val() + "/user/account/my-saved-items";
            }
          );
        } else if (
          data.status == "error" &&
          data.notice_type == "user_login_required"
        ) {
          swal({
            title: "",
          ...
          data.status == "error" &&
          data.notice_type == "item_already_exists"
        ) {
          swal(
            {
              title: "",
  ...
  setProductCompareData: function (id) {
    $.ajax({
      ...
      success: function (data) {
        if (
          data.status == "success" &&
          data.notice_type == "compare_data_saved"
        ) {
          swal(
            {
              title: "",
          ...
          data.status == "error" &&
          data.notice_type == "already_saved"
        ) {
          swal(
            {
              title: "",
          ...
          data.status == "error" &&
          data.notice_type == "compare_data_exceed_limit"
        ) {
          swal({
            title: "",
...
function product_review_tab_control_with_hashtag() {
  $("#product_single_page")
    .find(".product-description-bottom-tab ul.nav-tabs li.active")
    .removeClass("active");
  $("#product_single_page")
    .find(".product-description-bottom-tab ul.nav-tabs li:last-child")
    .addClass("active");
  $("#product_single_page")
    .find(".product-description-bottom-tab .tab-content .active")
    .removeClass("active");
  $("#product_single_page")
    .find(".product-description-bottom-tab .tab-content .in")
    .removeClass("in");
  $("#product_single_page")
    .find(".product-description-bottom-tab .tab-content #reviews")
    .addClass("active in");
}
...
function checkoutStepValidation() {
  ...
  var visible_step = $("#checkout_page .checkout-content .step:visible");
  var msgStr =
    '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">Ã—</button><div class="message-header"><i class="fa fa-exclamation-triangle"></i>&nbsp;<strong>' +
    frontendLocalizationString.error_message_text +
    '</strong></div><p class="error-msg"></p></div>';
  var emailMsg = "";
  ...
  if (visible_step.attr("id") && visible_step.attr("id") == "user_mode") {
    if ($("#selected_user_mode").val().length == 0) {
      $("#" + visible_step.attr("id"))
        .find(".checkout-process-user-mode")
        .before(msgStr);
      $("#" + visible_step.attr("id"))
        .find(".error-msg")
        .html(frontendLocalizationString.user_mode_select_msg);
      returnVal = false;
    }
  } else if (
    visible_step.attr("id") &&
    visible_step.attr("id") == "guest_user_address"
  ) {
    var errorStr = [];
    var isChecked = $("#different_shipping_address").prop("checked");
    if (
      $("#account_bill_first_name").length > 0 &&
      $("#account_bill_first_name").val().length == 0 &&
      $("#account_bill_first_name").val() == ""
    ) {
      errorStr.push("no_account_bill_first_name");
    }
    if (
      isChecked &&
      $("#account_shipping_first_name").length > 0 &&
      $("#account_shipping_first_name").val().length == 0 &&
      $("#account_shipping_first_name").val() == ""
    ) {
      errorStr.push("no_account_shipping_first_name");
    }
    if (
      $("#account_bill_last_name").length > 0 &&
      $("#account_bill_last_name").val().length == 0 &&
      $("#account_bill_last_name").val() == ""
    ) {
      errorStr.push("no_account_bill_last_name");
    }
    if (
      isChecked &&
      $("#account_shipping_last_name").length > 0 &&
      $("#account_shipping_last_name").val().length == 0 &&
      $("#account_shipping_last_name").val() == ""
    ) {
      errorStr.push("no_account_shipping_last_name");
    }
    if (
      $("#account_bill_email_address").length > 0 &&
      $("#account_bill_email_address").val().length == 0 &&
      $("#account_bill_email_address").val() == ""
    ) {
      errorStr.push("no_account_bill_email_address");
    }
    if (
      isChecked &&
      $("#account_shipping_email_address").length > 0 &&
      $("#account_shipping_email_address").val().length == 0 &&
    ...
    if (
      $("#account_bill_phone_number").length > 0 &&
      $("#account_bill_phone_number").val().length == 0 &&
      $("#account_bill_phone_number").val() == ""
    ) {
    ...
    if (
      isChecked &&
      $("#account_shipping_phone_number").length > 0 &&
      $("#account_shipping_phone_number").val().length == 0 &&
    ...
    if (isChecked && $("#account_shipping_select_country").length > 0) {
      get_shipping_country_name = $(
        "#account_shipping_select_country :selected"
      ).val();
    }
    ...
    if (
      $("#account_bill_adddress_line_1").length > 0 &&
      $("#account_bill_adddress_line_1").val().length == 0 &&
      $("#account_bill_adddress_line_1").val() == ""
    ) {
      errorStr.push("no_account_bill_adddress_line_1");
    }
    if (
      isChecked &&
      $("#account_shipping_adddress_line_1").length > 0 &&
      $("#account_shipping_adddress_line_1").val().length == 0 &&
      $("#account_shipping_adddress_line_1").val() == ""
    ) {
      errorStr.push("no_account_shipping_adddress_line_1");
    }
    if (
      $("#account_bill_town_or_city").length > 0 &&
      $("#account_bill_town_or_city").val().length == 0 &&
      $("#account_bill_town_or_city").val() == ""
    ) {
      errorStr.push("no_account_bill_town_or_city");
    }
    if (
      isChecked &&
      $("#account_shipping_town_or_city").length > 0 &&
      $("#account_shipping_town_or_city").val().length == 0 &&
    ...
    if (
      $("#account_bill_zip_or_postal_code").length > 0 &&
      $("#account_bill_zip_or_postal_code").val().length == 0 &&
      $("#account_bill_zip_or_postal_code").val() == ""
    ) {
    ...
    if (
      isChecked &&
      $("#account_shipping_zip_or_postal_code").length > 0 &&
      $("#account_shipping_zip_or_postal_code").val().length == 0 &&
    ...
    if (
      $("#account_bill_email_address").val().length > 0 &&
      !isValidEmail($("#account_bill_email_address").val())
    ) {
      emailMsg +=
        "<p>" + frontendLocalizationString.billing_email_not_valid_msg + "</p>";
    }
    if (
      isChecked &&
      $("#account_shipping_email_address").val().length > 0 &&
      !isValidEmail($("#account_shipping_email_address").val())
    ) {
      emailMsg +=
        "<p>" +
        frontendLocalizationString.shipping_email_not_valid_msg +
        "</p>";
    }
    if (errorStr.length > 0) {
      $("#" + visible_step.attr("id"))
        .find(".user-address-content")
        .before(msgStr);
      $("#" + visible_step.attr("id"))
        .find(".error-msg")
        .html(frontendLocalizationString.required_field_still_empty_msg);
      returnVal = false;
    }
    ...
    if ($("#selected_payment_method").val().length == 0) {
      ...
      $("#" + visible_step.attr("id"))
        .find(".error-msg")
        .html(frontendLocalizationString.select_payment_method_msg);
      returnVal = false;
    } else if (
      $("#selected_payment_method").val().length > 0 &&
      $("#selected_payment_method").val() == "stripe"
    ) {
      var errorStr = [];
      ...
      if (errorStr.length == 0) {
        ...
        if (
          parseStripeApiData.secret_key &&
          parseStripeApiData.secret_key != null &&
          parseStripeApiData.secret_key != "" &&
          parseStripeApiData.publishable_key &&
          parseStripeApiData.publishable_key != null &&
          parseStripeApiData.publishable_key != ""
        ) {
          ...
          $("#" + visible_step.attr("id"))
            .find(".error-msg")
            .html(frontendLocalizationString.stripe_api_empty_msg);
          returnVal = false;
        }
      }
    } else if (
      $("#selected_payment_method").val().length > 0 &&
      $("#selected_payment_method").val() == "2checkout"
    ) {
      var errorStr = [];
      ...
      if (errorStr.length == 0) {
        var parseTwoCheckoutApiData = JSON.parse(
          $("#2checkout_api_data").val()
        );
        if (
          parseTwoCheckoutApiData.sellerId &&
          parseTwoCheckoutApiData.sellerId != null &&
          parseTwoCheckoutApiData.sellerId != "" &&
          parseTwoCheckoutApiData.publishableKey &&
          parseTwoCheckoutApiData.publishableKey != null &&
          parseTwoCheckoutApiData.publishableKey != ""
        ) {
          var env = "production";
          if (
            parseTwoCheckoutApiData.sandbox_enable_option &&
            parseTwoCheckoutApiData.sandbox_enable_option == "yes"
          ) {
            env = "sandbox";
          }
          TCO.loadPubKey(env);
          ...
          TCO.requestToken(
            twoCheckoutSuccessCallback,
            twoCheckoutErrorCallback,
            args
          );
          $("#checkout_page .action").addClass("loading");
          returnVal = false;
          ...
          $("#" + visible_step.attr("id"))
            .find(".error-msg")
            .html(frontendLocalizationString.twocheckout_api_empty_msg);
          returnVal = false;
        }
      }
    }
    ...
    visible_step.attr("id") &&
    visible_step.attr("id") == "authentication"
  ) {
    if ($("#is_user_login").val() == false) {
      $("#" + visible_step.attr("id"))
        .find(".user-login-content")
        .before(msgStr);
      $("#" + visible_step.attr("id"))
        .find(".error-msg")
        .html(frontendLocalizationString.signup_signin_checkout_msg);
      returnVal = false;
    }
  } else if (
    visible_step.attr("id") &&
    visible_step.attr("id") == "login_user_address"
  ) {
    if (
      $("#is_login_user_address_exists").length == 0 &&
      $("#is_login_user_address_exists").val() != "address_added"
    ) {
      $("#" + visible_step.attr("id"))
        .find(".user-address-content")
        .before(msgStr);
      $("#" + visible_step.attr("id"))
        .find(".error-msg")
        .html(frontendLocalizationString.billing_shipping_error_msg);
      returnVal = false;
    }
  }
  return returnVal;
...
function twoCheckoutSuccessCallback(data) {
  ...
  $("form").append(
    "<input type='hidden' name='twoCheckoutToken' value='" + token + "'/>"
  );
  $("#checkout_page .checkout-content").find("#payment").hide();
  $("#checkout_page .checkout-content").find(".action").hide();
  $("#checkout_page .checkout-content").find("#order_notes").show();
