...
<body>
  ...
  <script>
    $("#srch_term").on('keyup', () =>{
      var product_title = $("#srch_term").val();
      var routeName = '{{ route("getProductByTitle") }}';
        ...
        $.ajax({
          ...
          success:function(response){
            if(response.products.length > 0){
              ...
              $(".search-menud").addClass('show');
              var data = '';
              $.each(response.products, (key, value) =>{
                var productDetailRoute = '{{ route("details-page", ":slug") }}';
                    productDetailRoute = productDetailRoute.replace(":slug", value.slug);
                if(value.image_url == ''){
                  var image_url = base_url + '/public/images/no-image.png';
                } else {
                  var image_url = base_url + value.image_url;
                }
                data += `<a href="`+ productDetailRoute +`" style="text-decoration: none;">
                        <li class="dropdown-item">
                            <img class="img-thumbnail" height="100px" src="`+image_url+`"> `+ value.title +`
                        </li>
                    </a>`;
                
              });
              $(".search-menud").html(data);
            } else {
              $(".searchBox").removeClass('show');
              $(".search-menud").removeClass('show');
