...
class GetFunction
{
    ...
    public function sendCustomMail($data)
    {
        ...
        // dd($this->option->getPaymentMethodData());
        ...
        if ($data['source'] == 'order_confirmation') {
            ...
            if ($get_post_meta && $get_post_meta->count() > 0) {
                ...
                foreach ($get_post_meta as $rows) {
                    if ($rows->key_name == '_order_currency' ||
                        $rows->key_name == '_customer_ip_address' ||
                        $rows->key_name == '_customer_user_agent' ||
                        $rows->key_name == '_customer_user' ||
                        $rows->key_name == '_order_shipping_cost' ||
                        $rows->key_name == '_order_shipping_method' ||
                        $rows->key_name == '_payment_method' ||
                        $rows->key_name == '_payment_method_title' ||
                        $rows->key_name == '_order_tax' ||
                        $rows->key_name == '_order_total' ||
                        $rows->key_name == '_order_notes' ||
                        $rows->key_name == '_order_status') {
            ...
            if ($get_post_meta && $get_post_meta->count() > 0) {
                ...
                foreach ($get_post_meta as $rows) {
                  // dd($rows->key_name);
                    if ($rows->key_name == '_order_currency' || $rows->key_name == '_customer_ip_address' ||
                        $rows->key_name == '_customer_user_agent' || $rows->key_name == '_customer_user' ||
                        $rows->key_name == '_order_shipping_cost' || $rows->key_name == '_order_shipping_method' ||
                        $rows->key_name == '_payment_method' || $rows->key_name == '_payment_method_title' ||
                        $rows->key_name == '_order_tax' || $rows->key_name == '_order_total' ||
                        $rows->key_name == '_order_notes' || $rows->key_name == '_order_status') {
                        $get_view_data[$rows->key_name] = $rows->key_value;
                    }
                }
                // dd($get_view_data);
                $payment = $this->option->getPaymentMethodData();
                // dd($this->option->getPaymentMethodData()[$get_view_data['_payment_method']]);
